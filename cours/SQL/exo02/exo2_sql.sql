...
Question14----
select commande.ncmd, extract (month from cmd_date) as month_cmd
 from commande
where extract (month from cmd_date) = 8

Quesstion15--
select   rep_nom ||' '|| rep_prenom As Représentant, pro_libelle, cli_societe
...
inner join detail on commande.ncmd = detail.ncmd
inner join produit on detail.nprod= produit.nprod
where rep_nom = 'LARRY FITCH' and pro_libelle = 'WIDGET INSTALLER'

16Question16---

select nbur, count(nbur) as nbr from representant   ---https://www.sqlfacile.com/apprendre_bases_de_donnees/count_et_group_by
group by nbur 
having count(nbur) > 2

Question18--
select  rep_nom ||' '|| rep_prenom as nom_du_epesentant, count(commande.ncmd)
	from ville
		inner join bureau on bureau.cp = ville.cp 
		inner join representant on bureau.nbur = representant.nbur
		inner join origine on representant.nrep = origine.nrep
		inner join commande on origine.ncmd = commande.ncmd
		where vil_nom = 'CHICAGO'
group by  nom_du_epesentant
having count(commande.ncmd) >1
